...
# A_d = {}#[[]] # transition 
# B_d = {}#[[]] # emmision/observation matrix(dictionary)
# for i in all_possible_hidden_states:
#     A_d[i] = robot.Distribution(transition_model(i))
#     B_d[i] = robot.Distribution(observation_model(i))

A = np.array([[float(0)]*len(all_possible_hidden_states)]* len(all_possible_hidden_states))
B = np.array([[float(0)]*len(all_possible_observed_states)] *len(all_possible_hidden_states))
# A[1,:] = 1
# A[0,:] = 2
...
#     print(sum(B[i,:]))
# print(A[:10,:10])
# print(all_possible_observed_states[:2])
# for i, state in enumerate(all_possible_hidden_states):
np.set_printoptions(precision=3, linewidth=200) 
# print(transition_matrix[:10, :10], '\n') 
# print(B[:10, :10], '\n')
# print(obs_state_index_map[(0,0)])
# print(B[:10,:10])
...
def Viterbi(observations, second_best=False):
    ...
    for i, x_i in enumerate(observations):
        ...
        if i == 0 and x_i:
            ...
            prev = np.array([mes[i-1][state] for state in all_possible_hidden_states])
            # print(prev,'\n')
            for s_i, state in enumerate(all_possible_hidden_states):
                cur = log_A[:,s_i] + prev
                t[state] = all_possible_hidden_states[np.argmin(cur)]
            ...
            for s_i, state in enumerate(all_possible_hidden_states):
                ...
                t[state] = all_possible_hidden_states[np.argmin(cur)]
                m[state] = cur.min()
                # m[state] = np.min(log_A[s_i,:] + phi + prev)
    ...
    last_message = np.array([mes[-1][state] for state in all_possible_hidden_states])
    x_hat = all_possible_hidden_states[np.argmin(last_message + log_B[:,last_ob_index])]
    ...
    # print('pre-break list ', l[::-1])
    viterbi = l[::-1]
    # viterbi.insert(0, viterbi[0])
    # print()
    ...
    # print('\nExpected: ',[[2, 6, "stay"], [3, 6, "right"], [4, 6, "right"], [5, 6, "right"], [6, 6, "right"], [6, 6, "stay"], [6, 5, "up"], [6, 5, "stay"], [5, 5, "left"], [4, 5, "left"]])
    # print('\nExpected:', [[1, 0, "stay"], [1, 0, "stay"], [2, 0, "right"], [3, 0, "right"], [4, 0, "right"], [5, 0, "right"], [6, 0, "right"], [6, 0, "stay"], [6, 1, "down"], [6, 2, "down"]])
    # print(viterbi[-3])
    return viterbi#estimated_hidden_states
...
def second_best(observations):
    ...
    # mes[0] = np.array([-np.log2(i) if i != 0 else np.inf for i in prior_matrix])
    # mes2[0] = np.array([-np.log2(i) if i != 0 else np.inf for i in prior_matrix])
    # for i, x_i in enumerate(observations):
    #     if x_i:
    #         obs_i = obs_state_index_map[x_i]
    #         phi = log_B[:, obs_i]
    ...
    #     elif x_i is None:
    #         prev = np.array([mes[i-1][state] for state in all_possible_hidden_states])
    #         prev2 = np.array([mes2[i-1][state] for state in all_possible_hidden_states])
    #         # print(prev2)
    #         for s_i, state in enumerate(all_possible_hidden_states):
    #             cur = log_A[:,s_i] + prev
    #             t[state] = all_possible_hidden_states[np.argmin(cur)]
    ...
    # last_ob_index = obs_state_index_map[observations[-1]]
    # last_message = np.array([mes2[-1][state] for state in all_possible_hidden_states])
    # # last_message[]
    # x_hat = all_possible_hidden_states[np.argmin(last_message + log_B[:, last_ob_index])]
