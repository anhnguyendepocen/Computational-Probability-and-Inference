...
def compute_posterior(prior, likelihood, y):
    ...
    P_y_x = np.log(likelihood[y,:]).sum(axis=0)
    
    log_num = P_y_x+log_prior
    log_denom = P_y_x+log_prior
    
    log_answer = log_num-scipy.misc.logsumexp(log_denom)
...
def compute_movie_rating_likelihood(M):
    ...
    for row in range(M):
        for col in range(M):
            if row==col:
                likelihood[row, col] = 2
            else:
                likelihood[row, col] = 1 / abs(row-col)
    
    alpha = 1/likelihood.sum(axis=1)#sum rows
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for i in range(num_movies):
        if num_observations==-1:
            posteriors[i] = compute_posterior(prior,likelihood,
                movie_data_helper.get_ratings(movie_id_list[i]))
        else:
            posteriors[i] = compute_posterior(prior,likelihood,
                movie_data_helper.get_ratings(
                                        movie_id_list[i])[:num_observations])
    # print(movie_data_helper.get_movie_name(1))
    # print(movie_data_helper.get_ratings(movie_id_list[1])[:num_observations])

    for movie in range(num_movies):
        MAP_ratings[movie] = np.argmax(posteriors[movie])

    top_10 = np.argsort(MAP_ratings)[::-1][:10]
    top_10_names = [movie_data_helper.get_movie_name(i) for i in top_10]
    # print(sum(MAP_ratings==10))
    # print(MAP_ratings[368])
    # print(top_10_names)
...
def compute_entropy(distribution):
    ...
    # -------------------------------------------------------------------------
    e = lambda p: np.sum(p * np.log2(1 / p))##entropy formula
    entropy = np.array([e(p) if p>10e-8 else 0 for p in distribution]).sum()
    #
    # Be sure to:
    # - use log base 2
    # - enforce 0log0 = 0
...
def compute_true_movie_rating_posterior_entropies(num_observations=-1):
    """
    ...
    # print(posteriors[0])
    # print(compute_entropy(posteriors[0]))
    posterior_entropies = np.array([compute_entropy(p) for p in posteriors])
    # print(posterior_entropies)
...
def main():
    ...
    y = [0]*2 + [1]*1
    my_answer = compute_posterior(prior, likelihood, y)
    expected_answer = np.array([[0.91986917, 0.08013083]])
    print("My answer:")
    print(my_answer)
    ...
    compute_true_movie_rating_posterior_entropies(-1)
    n=200
    posts = [compute_true_movie_rating_posterior_entropies(i).mean() 
             for i in range(n)] 
    N = np.array(range(n))
    plt.plot(posts)
    plt.show()

    # compute_movie_rating_likelihood(9)
